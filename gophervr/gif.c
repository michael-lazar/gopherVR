  ...
  int s;
{
  ...
  if ((tmp = read(ins, buf, GIF_SIG_LEN)) != GIF_SIG_LEN)
  ...
  if (read(ins, buf, GIF_SD_SIZE) != GIF_SD_SIZE)
    return GIFIN_ERR_EOF;
...
/*    separator = getc(ins);*/
  ...
  if (read(ins, buf, GIF_ID_SIZE) != GIF_ID_SIZE)
    return GIFIN_ERR_EOF;
...
/*  buf_cnt = zgetc(ins);*/
  ...
  if (read(ins, buf, buf_cnt) != buf_cnt)
    return GIFIN_ERR_EOF;
...
main()
{
     ...
     x = open("/home/arcwelder/lindner/surfgopher.gif", O_RDONLY);
     if (x < 0) {
	  perror("Could not open file\n");
	  exit(-1);
     }
 
     if ((gifin_open_file(x) != GIFIN_SUCCESS) || /* read GIF header */
